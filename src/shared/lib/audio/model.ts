...
import {
  calculateStatistics,
  prepareStratifiedSplit,
  prepareLeaveOneGuitarOutSplit,
  SEQUENCE_LENGTH,
  NUM_FEATURES,
...
export async function trainModel(
  ...
  if (holdOut) {
    console.log(
      `Leave-one-guitar-out split: validating on "${holdOut}", training on the rest`,
    );
  }
    ...
    callbacks: [
      ...
      new tf.CustomCallback({
        onEpochEnd: (epoch, logs) => {
          console.log(
            `Epoch ${epoch + 1}: Precisión Entrenamiento = ${(logs!.acc * 100).toFixed(1)}% | Precisión Validación = ${(logs!.val_acc * 100).toFixed(1)}%`,
          );
        },
      }),
    ],
  ...
  const manifestEntry: ModelManifestEntry = {
    ...
    ...(holdOut
      ? {
          notes: `LOGO experiment: validated on held-out guitar "${holdOut}" — for measurement, not deployment`,
        }
      : {}),
  ...
  console.log(
    "Manifest entry downloaded — paste it under modes.default in public/model/manifest.json",
  );
