...
> *Traducción de [audio-pipeline.md](audio-pipeline.md), sincronizada a 2026-07-13. Ante cualquier discrepancia, el original en inglés es la referencia.*
...
| `src/shared/lib/audio/prediction-stabilizer.ts` | Ventana de votos pura del lado de decodificación: mayoría con reinicio por onset + autolimpieza por silencio (con tests unitarios) |
...
6. **Estabilización con RxJS** (`prediction-stabilizer.ts`): las predicciones crudas pasan por una ventana deslizante de 5 votos; un par (cuerda, traste) debe ganar ≥70% para emitirse, y una predicción emitida se limpia sola tras 5 s de silencio. **La ventana de votos se reinicia con cada onset de pulsación** — de conservarse, los votos de la nota anterior (posiblemente aún sonando) tendrían que ser superados por la nueva pulsación (~100 ms de latencia extra, y un bloqueo total cuando el rastreador monofónico alterna entre dos cuerdas sonando a la vez). Tras un reinicio la mayoría puede resolverse sobre una ventana parcial (≥3 votos, unanimidad hasta que se rellena), así que las pulsaciones nuevas emiten rápido.
...
| **Ventana de votos con reinicio por onset** (estabilizador extraído a `prediction-stabilizer.ts`) | Los votos rancios de una nota aún sonando tenían que ser superados por una pulsación nueva (≥4/5): ~100 ms de latencia extra al cambiar, y un bloqueo total cuando el rastreador monofónico alternaba entre dos cuerdas sonando. Un onset ahora vacía la ventana, y una ventana parcial unánime fresca (≥3 votos) puede emitir antes |
...
4. **Sustracción espectral en el onset para transiciones con notas sonando**: mantener en el worker de features una EMA corta del espectro de magnitud previo al onset; durante unos cientos de ms tras cada onset, restarla (con una estimación de decaimiento por frame) del espectro analizado, de modo que la detección de pitch y el seguimiento de parciales vean sobre todo la energía de la nota *nueva*. Debe ejecutarse idéntica en captura e inferencia (simetría entrenamiento/inferencia), así que hay que emparejarla con una variante "déjala sonar" del runner guiado y un reentrenamiento. Los parciales compartidos (octavas, quintas) siguen siendo ambiguos — esto reduce la necesidad de apagar cuerdas, no la elimina. La ventana de votos con reinicio por onset (§7.6) ya elimina la parte del problema del lado de decodificación.
