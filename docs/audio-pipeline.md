...
| `src/shared/lib/audio/prediction-stabilizer.ts` | Pure decode-side vote window: onset-reset majority + silence auto-clear (unit-tested) |
...
6. **RxJS stabilization** (`prediction-stabilizer.ts`): raw predictions pass through a sliding 5-vote window; a (string, fret) pair must win ≥70% to emit, and an emitted prediction auto-clears after 5 s of silence. **The vote window resets on every pluck onset** — kept, the previous (possibly still-ringing) note's votes would have to be out-voted by the new pluck (~100 ms of flip latency, and a deadlock when the monophonic tracker alternates between two sounding strings). After a reset the majority may resolve on a partial window (≥3 votes, unanimity until it refills), so fresh plucks emit fast.
...
| **Onset-reset vote window** (stabilizer extracted to `prediction-stabilizer.ts`) | Stale votes for a still-ringing note had to be out-voted by a new pluck (≥4/5): ~100 ms extra flip latency, and a deadlock when the monophonic tracker alternated between two sounding strings. A pluck onset now clears the window, and a fresh unanimous partial window (≥3 votes) may emit early |
...
4. **Onset spectral subtraction for ringing-note transitions**: keep a short EMA of the pre-onset magnitude spectrum in the feature worker; for a few hundred ms after each onset, subtract it (with a per-frame decay estimate) from the analyzed spectrum so pitch detection and partial tracking see mostly the *new* note's energy. Must run identically at capture and inference (train/inference symmetry), so pair it with a guided-runner "let it ring" recording variant and a retrain. Shared partials (octaves, fifths) stay ambiguous — this narrows the muting requirement, it doesn't remove it. The onset-reset vote window (§7.6) already removes the decode-side part of the problem.
